                   ...
                   MPI_Comm comm)
{
  ...
  double left_ghost = ukt[0];
  double right_ghost = ukt[Nx-1];
  double ldata;
  double rdata;
  for (size_t step=0; step<Nt; ++step){
      MPI_Barrier(comm);
      ...
      MPI_Isend(&right_ghost, 1, MPI_DOUBLE, right_process, 0, MPI_COMM_WORLD, &req);
      MPI_Recv(&ldata, 1, MPI_DOUBLE, left_process, 0, MPI_COMM_WORLD, &stat);  
      uktp1[0] = ukt[0] + nu*(ldata - 2*ukt[0] + ukt[1]);// compute the first one, need the element from ukt[nx-1] from left(left_gohst)  

      MPI_Isend(&left_ghost, 1, MPI_DOUBLE, left_process, 0, MPI_COMM_WORLD, &req);
      MPI_Recv(&rdata, 1, MPI_DOUBLE, right_process, 0, MPI_COMM_WORLD, &stat); 
      uktp1[Nx-1] = ukt[Nx-1] + nu*(ukt[Nx-2] - 2*ukt[Nx-1] + rdata); // compute the last one, need the element from ukt[0] from right (right ghost)
